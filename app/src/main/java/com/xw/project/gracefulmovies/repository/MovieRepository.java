...
public class MovieRepository {
    ...
    public LiveData<DataResource<List<MovieEntity>>> getMovieNowList(int cityId) {
        return new NetworkBoundResource<List<MovieEntity>>() {
            @NonNull
            @Override
            protected LiveData<List<MovieEntity>> loadFromLocal(MutableLiveData<List<MovieEntity>> mutableLiveData) {
                Observable.just(mutableLiveData)
                        .compose(RxSchedulers.applyIO())
                        .subscribe(new SimpleConsumer<MutableLiveData<List<MovieEntity>>>() {
                            @Override
                            public void accept(MutableLiveData<List<MovieEntity>> it) {
                                List<MovieEntity> list = GMApplication.getInstance().getDatabase()
                                        .movieDao().loadMovieNowList();
                                it.postValue(list);
                            }
                        });
